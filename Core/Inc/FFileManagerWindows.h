...
// File manager.
class CORE_API FArchiveFileReader : public FArchive
{
	// TODO: The class methods have not been edited to match DNF yet,
	// but the internal field layout should be correct. Luckily, this
	// class is dllexport'd by dnCommon.dll, so we don't need an impl.
public:
...
{
public:
	FArchive* CreateLinearReader( const TCHAR* Filename )
	{
		return CreateFileReader( Filename, 0, GError );
	}
	...
	TArray<FString> FindFiles( const TCHAR* Filename, UBOOL Files, UBOOL Directories, UBOOL SubDirectories, UBOOL Absolute )
	{
		...
		TArray<dnDirectoryInfo> DirInfo;
		HANDLE Handle=NULL;
		WIN32_FIND_DATA Data;
		UBOOL AddedDirectory;

		if( SubDirectories )
		{
			Directory = GetDefaultDirectory();
			FindSubDirectories( DirInfo, 1 );

			for( INT i=0; i<DirInfo.Num(); i++ )
			{
				dnDirectoryInfo& SubDir = DirInfo(i);
				AddedDirectory = 0;
				SetDefaultDirectory( SubDir.Unknown2 );
				...
				do
				{
					if( (Data.dwFileAttributes&FILE_ATTRIBUTE_DIRECTORY)?Directories:Files )
					{
						if( Directories && !AddedDirectory )
						{
							if( Absolute )
							{
								new(Result)FString(SubDir.Unknown2);
							}
							else
							{
								new(Result)FString(SubDir.Unknown1);
							}
	...
	void FindSubDirectories( TArray<dnDirectoryInfo>& SubDirs, UBOOL Root )
	{
		FString Str;
		HANDLE Handle;
		WIN32_FIND_DATA Data;
		dnDirectoryInfo Info;
		Handle = FindFirstFile( TEXT("*.*"), &Data );
		...
		if (Handle != INVALID_HANDLE_VALUE)
		{
			do
			{
				if( Data.dwFileAttributes&FILE_ATTRIBUTE_DIRECTORY )
				{
					if( !appStricmp(Data.cFileName, TEXT("..")) || (!Root && !appStricmp(Data.cFileName, TEXT("."))) )
						continue;

					Str = GetDefaultDirectory();
					Str += Data.cFileName;
					Str += TEXT("\\");
					appStrncpy( Info.Unknown1, Data.cFileName, ARRAYSIZE(Info.Unknown1) );
					Info.Unknown1[ARRAYSIZE(Info.Unknown1)-1] = TEXT('\0');
					appStrncpy( Info.Unknown2, *Str, ARRAYSIZE(Info.Unknown2) );
					Info.Unknown2[ARRAYSIZE(Info.Unknown2)-1] = TEXT('\0');
					SubDirs.AddItem( Info );

					if( appStricmp( Data.cFileName, TEXT(".") ) )
					{
						SetCurrentDirectory( *Str );
						FindSubDirectories( SubDirs, 0 );
					}
				}
