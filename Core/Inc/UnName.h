...
#if DNF
	UBOOL Contains( const TCHAR* );
	const dnStringA ToAnsi2() const;
	const TCHAR* GetValid() const;
	static int GetNumNames();
	static TArray<FNameEntry*>& GetPureNamesTable();
	static void ReleaseNamesTable();
	static void SetAssertOnConstruction( UBOOL );
	static UBOOL GetAssertOnConstruction();
#endif
...
#if DNF
	//static dnThread_Semaphore NamesCS;
#endif
};
...
#if DNF
class CORE_API dnNameA
{
public:
	UBOOL Contains( const char* );
	NAME_INDEX GetIndex() const;
	UBOOL IsValid() const;
	const char* Str() const;
	const dnStringA& StrA() const;
private:
	NAME_INDEX Index;
};
#endif
