...
class CORE_API FBitArray
{
public:
	FBitArray() : Data( NULL ), NumBits( 0 ) {}
	FBitArray( INT );
	UBOOL GetBit( INT ) const;
	void SetBit( INT, UBOOL );
	void SetNewMaxBits( INT );
	void Zero()
	{
		if( Data )
		{
			appMemset( Data, 0, NumBits / 8 + 1 );
		}
	}
	void DestroyBitArray()
	{
		if( Data )
		{
			appFree( Data );
	...
	{
		Ar.ByteOrderSerialize( &A.NumBits, sizeof(A.NumBits) );

		if( Ar.IsLoading() )
			A.SetNewMaxBits( A.NumBits );

		if( A.NumBits )
		{
			for( INT Index=0; Index < A.NumBits / 8 + 1; Index++ )
			{
				Ar.Serialize( A.Data + Index, 1 );
			}
		}
...
#if DNF
class CORE_API dnStringA : protected TArray<ANSICHAR>
{
};
#endif
...
#undef TMap
#undef TMultiMap
template<class TK, class TI> class TMap : public dnMap<TK, TI> {};
template<class TK, class TI> class TMultiMap : public dnMultiMap<TK, TI> {};
#endif
