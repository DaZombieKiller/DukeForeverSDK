...
public:
	...
	{
	#if DNF
		_sntprintf( Dest, Size, TEXT("%08X%08X%08X%08X"), A, B, C, D );
	#else
...
public:
...
#if DNF
	virtual UBOOL SupportsUserText() {return 0;}
	virtual void ExportUserText( FOutputDevice& ) {}
	virtual UBOOL ImportUserText( const TCHAR*&, const TCHAR* ) {return 0;}
	virtual UBOOL IsPendingKill() const {return 0;}
...
#if DNF
	virtual void Register( UBOOL );
#else
	virtual void Register();
#endif
	virtual void LanguageChange();
#if DNF
	virtual void PostEditChange( UProperty* ) { Modify(); }
	virtual void RestoreFromUndo() { PostEditChange(NULL); }
...
#if DNF
	static UObject* StaticConstructObject( UClass* Class, UObject* InOuter=(UObject*)GetTransientPackage(), const FName& Name=NAME_None, DWORD SetFlags=0, UObject* Template=NULL, FOutputDevice* Error=GError, UObject* Ptr=NULL );
	static void StaticEnterSafeMode( const TCHAR* );
#else
...
#if DNF
	static void SetLanguage( const TCHAR* LanguageExt, UBOOL Unknown=1 );
#else
	...
	void RemoveFromRoot();
#if DNF
	const dnString GetFullName( TCHAR* Str=NULL ) const;
	const dnString GetPathName( UObject* StopOuter=NULL, TCHAR* Str=NULL ) const;
#else
